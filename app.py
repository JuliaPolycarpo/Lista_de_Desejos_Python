...
app.config["UPLOAD_FOLDER"] = UPLOAD_FOLDER # configuro o caminho da pasta onde as imagens serão salvas. O "app.config" é um dicionário usado para armazenar configurações da aplicação Flask.
...
def lista():
    if request.method == 'POST':
        ...
        if imagem and imagem.filename != "": # verifica se o arquivo existe e se tem um nome válido]
            nome_imagem = imagem.filename  #nome_imagem = secure_filename(imagem.filename) # gera um nome seguro para o arquivo
...
def update(idDesejo):
    if request.method == 'POST':
        ...
        if imagem and imagem.filename != "": # verifica se o arquivo existe e se tem um nome válido
            nome_imagem = imagem.filename # gera um nome seguro para o arquivo
            caminho = os.path.join(app.config["UPLOAD_FOLDER"], nome_imagem) # cria o caminho completo para salvar a imagem. O "os.path.join" é usado para garantir que o caminho seja construído corretamente, independentemente do sistema operacional.
            imagem.save(caminho) # salva a imagem no servidor
        else:
